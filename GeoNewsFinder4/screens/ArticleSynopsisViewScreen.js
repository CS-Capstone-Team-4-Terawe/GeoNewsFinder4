...
// import { ask, summarize, NewsArticle } from '../utils/openAIGPTFunctions'; 
import { NewsArticle } from '../utils/openAIGPTFunctions'; 
...
const ArticleSynopsisView = () => {
  ...
  const [summary, setSummary] = useState('Loading...');
  // const [output, setSummary] = useState('Loading...');
  const summarize =  async () => {
    setIsLoading(true);
		setError(null);
    console.log("POGGERS 1");

    let user_name = "Ethan"
    console.log("POGGERS 2");
    ...
    conversation.push({ role: "user", content: initial_prompt });
    ...
    try{
      ...
      conversation.push({ role: "assistant", content: completion.data.choices[0].message.content });
